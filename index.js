...
$(function () {
  ...
  var fallProcess = function () {
    if (block.fallJudge()) { // 落ちれるなら
      block.fall();           // ↓におちる
    } else {
      block.fix();            // 落ちれないなら、ブロックを固定
      eraseJudge(block);    // 消える行あるか判定し、消します。詰めます。 を、行う。
      if (block.gameOverJudge()) { // ゲームオーバーなら・・
